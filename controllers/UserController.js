...
const RegisterUser = async (req, res) => {
    ...
    try {
      ...
      if ( user ){
        return res.status(500).json({ success: false, error: 'User already exist' });
      }
...
const LoginUser = async (req, res)=>{
    ...
    if(user){
        ...
        if (match){
            const accessToken = jwt.sign({ id: user._id, name}, process.env.ACCESS_KEY,{expiresIn: '1h' });
            const refreshToken = jwt.sign({ id: user._id}, process.env.REFRESH_KEY,{expiresIn: '5d' });
            ...
            return res.status(500).json({ success: false, error: 'Erreur de connexion' });
        }
    }else{
        return res.status(500).json({ success: false, error: 'Erreur de connexion' });
    }
...
const ProviderAuth = async (req, res)=>{
    ...
    if(user){
        ...
        if (match){
            const accessToken = jwt.sign({ id: loginUser._id, name}, process.env.ACCESS_KEY,{expiresIn: '1h' });
            const refreshToken = jwt.sign({ id: loginUser._id}, process.env.REFRESH_KEY,{expiresIn: '5d' });
            res.cookie('jwt', refreshToken, {
                httpOnly : true,
                maxAge:  5 * 24 * 60 * 60 * 1000,
                path: '/',
...
const RefreshUser= (req, res) =>{
    ...
    jwt.verify(
        ...
        async function (err, deccoded){
            ...
            const accessToken = jwt.sign({id: foundUser._id, name: foundUser.name}, process.env.ACCESS_KEY,{expiresIn: '1h'});
